...
public class EstadoService implements EstadoRepository{
	...
	public List<Estado> findAllByEstados(Long id) {
		res = new ArrayList<>();
		List<Estado> list_estados = estadoRepository.findAll();
		
		for (int i = 0; i < list_estados.size(); i++) {
			if (list_estados.get(i).getPais().getId()==id) {
				res.add(list_estados.get(i));
			}
		}
		return res;
	}
